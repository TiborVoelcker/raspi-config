...
#
# This is the one place in the repo that pipes curl into a shell. There is no
# Debian package, and the npm package needs Node 22 where trixie ships 20, so
# upstream's installer is the practical route on a Pi.
#
# It installs into a home directory rather than system-wide, so it runs as
# $SUDO_USER. Root would get its own copy in /root, which is not the one you
# would be typing `claude` at.
set -euo pipefail
...
# will when you log in. Cheaper than guessing where the installer put things.
...
# pipefail inside the inner shell too: without it a failed download still
# reports success, because the exit status would be bash's after reading
# nothing at all.
