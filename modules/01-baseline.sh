...
#
# Captured exactly once. The baseline only ever boots, resets and reboots, so
...
install -Dm755 "$REPO_DIR/reset/apply.sh" \
    "$BASELINE_ROOT_MNT/usr/local/sbin/homelab-apply-reset"
install -Dm644 "$REPO_DIR/lib/common.sh" "$BASELINE_ROOT_MNT$COMMON_LIB"
install -Dm644 "$REPO_DIR/reset/homelab-apply-reset.service" \
    "$BASELINE_ROOT_MNT/etc/systemd/system/homelab-apply-reset.service"
mkdir -p "$BASELINE_ROOT_MNT/etc/systemd/system/multi-user.target.wants"
ln -sfn /etc/systemd/system/homelab-apply-reset.service \
    "$BASELINE_ROOT_MNT/etc/systemd/system/multi-user.target.wants/homelab-apply-reset.service"
