...
once by `01-baseline.sh` before any other module runs, then left alone. No
...
| `lib/common.sh` | Sourced by everything, including `apply.sh` inside the baseline. Output helpers, guards, layout constants, copy primitives. |
| `modules/00-storage.sh` | Carves `p3`/`p4` out of unallocated space. |
| `modules/01-baseline.sh` | Captures the baseline onto `p3`/`p4`, once; writes `autoboot.txt`. |
...
| `reset/apply.sh` | Installed into the baseline as `/usr/local/sbin/homelab-apply-reset`. Overwrites `p1`/`p2` with itself, behind three gates. |
| `reset/homelab-apply-reset.service` | Runs `apply.sh` at boot on the baseline. |

**Module numbering is load-bearing.** `01-baseline.sh` captures the baseline, so
...
`01-baseline.sh` makes three changes to the clone: retarget its fstab at p3/p4,
write `baseline` to the role file, and install the reset machinery.
`reset/apply.sh` undoes exactly those on the way back, plus the cmdline.
...
- `apply.sh` runs from a copy of `lib/common.sh` installed at
