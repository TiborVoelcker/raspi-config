...
**Run it early.** `01-baseline.sh` captures the baseline from whatever state the
...
sudo homelab-reset           # wipe live, put the baseline back
```
...
`homelab-apply-reset` runs on every baseline boot and refuses to do anything unless
**all three** hold:
