...
# homelab-apply-reset.service. Overwrites the live system (bootA + rootA) with
...
# are what gets undone below, and anything added to 01-baseline.sh needs its
# inverse here.
#
...
echo "=== homelab-apply-reset $(date -Is) ==="
...
rm -f "$MAIN_ROOT_MNT/etc/systemd/system/multi-user.target.wants/homelab-apply-reset.service" \
      "$MAIN_ROOT_MNT/etc/systemd/system/homelab-apply-reset.service" \
      "$MAIN_ROOT_MNT/usr/local/sbin/homelab-apply-reset" \
